...
export default {
  ...
  methods: {
    ...
    initGraph() {
      ...
      let ratio = `${(this.graph.zoom() * 100).toFixed(0)}%`;
      this.$emit("changeratio", ratio);
    },
    initResize() {
    ...
    // 缩小
    zoomOut() {
      if (this.graph.zoom() >= 3 || this.graph.zoom() <= 0.4) {
        return;
      } else {
        this.graph.zoom(-0.1);
        let ratio = `${(this.graph.zoom() * 100).toFixed(0)}%`;
    ...
    save() {
      ...
      // 遍历元素，找到所有的组
      //   const groups = cells.filter(
      //     (cell) => cell.isNode() && cell.shape === "rect"
      //   );

      // 打印组的数据
      cells.forEach((group) => {
        console.log(group, "所以数据");
      });
    },
    getGraph() {
      return this.graph;
